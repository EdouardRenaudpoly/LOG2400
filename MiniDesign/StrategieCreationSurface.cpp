...
void StrategieSurfaceDistanceMin::relierPoints(std::vector<std::vector<char>> &grille, std::vector<std::shared_ptr<IAffichablePoint>> &points)
{
    ...
    while (nbVisites < points.size())
    {
        ...
        for (int i = 0; i < points.size(); i++)
        {
            if (!visite[i])
            {
                auto pa = points[indexCourant]->getPointDeBase();
                auto pb = points[i]->getPointDeBase();
                if (!pa || !pb)
                    continue;
        ...
        auto pa = points[indexCourant]->getPointDeBase();
        auto pb = points[suivant]->getPointDeBase();
        if (pa && pb)
            tracerLigne(grille, pa->x, pa->y, pb->x, pb->y);
    ...
    if (points[indexCourant]->getPointDeBase() && points[0]->getPointDeBase())
