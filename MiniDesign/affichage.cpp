...
void NuageDePoints::ajouterComposant(const shared_ptr<ComposanteAffichageAbs> &element)
{
    ...
    else
    {
        ...
        if (ipoint)
        {
            string textures = ipoint->getTextures();
            bool alreadyHasThisTexture = (!textures.empty() && textures.back() == this->texture);
            
            if (!alreadyHasThisTexture)
            {
...
void NuageDePoints::rajouterPoint(const shared_ptr<ComposanteAffichageAbs> &point)
{
    points.push_back(point);
    trierPointsParId();
}

void NuageDePoints::trierPointsParId()
{
    sort(points.begin(), points.end(),
         [](const shared_ptr<ComposanteAffichageAbs> &a, const shared_ptr<ComposanteAffichageAbs> &b)
         {
             auto ptA = dynamic_pointer_cast<IAffichablePoint>(a);
             auto ptB = dynamic_pointer_cast<IAffichablePoint>(b);
             
             if (ptA && ptB)
             {
                 auto baseA = ptA->getPointDeBase();
                 auto baseB = ptB->getPointDeBase();
                 if (baseA && baseB)
                     return baseA->getId() < baseB->getId();
             }
             return false;
         });
}
...
{
    cout << "Nuage ID " << nuageDePoints.getId() << ": avec la texture '" << nuageDePoints.texture << "' contient les points:";

    vector<shared_ptr<IAffichablePoint>> pts;
    pts = nuageDePoints.getPoints(pts);
    for (auto &&pt : pts)
    {
        cout << " " << pt->getPointDeBase()->getId() << ",";
    }
